...
const resumeDocumentPaths = {
  complete: {
    PT: "/curriculos/Wagner_Curriculo_.NET.pdf",
    EN: "/curriculos/Wagner_CV_English.pdf",
  },
  custom: {
    PT: "",
    EN: "",
...
const footerContent = {
  PT: {
    footer: {
      ...
      customSectionsOptionsEmpty:
        "Nenhuma opção extra disponível para as seções selecionadas.",
      customSectionsEmpty:
        "Selecione pelo menos uma seção para abrir o currículo personalizado.",
      resumeModes: {
        complete: {
          title: "Completo",
        ...
        custom: {
          title: "Personalizado",
          description:
            "Monta uma versão ajustada para uma vaga ou objetivo específico.",
        },
      },
      customSections: {
  ...
  EN: {
    footer: {
      ...
      customSectionsOptionsEmpty:
        "There are no extra options available for the selected sections.",
      customSectionsEmpty:
        "Select at least one section to open the custom resume.",
      resumeModes: {
        complete: {
          title: "Complete",
        ...
        custom: {
          title: "Custom",
          description:
            "Builds a version tailored to a specific role or objective.",
        },
      },
      customSections: {
...
export default function FooterSection({
  ...
  const [selectedCustomOptions, setSelectedCustomOptions] =
    useState(defaultCustomOptions);
  ...
  const selectedResumeUrl = useMemo(() => {
    ...
    return (
      resumeDocumentPaths[selectedResumeMode]?.[selectedResumeLanguage] ?? "#"
    );
  }, [selectedResumeLanguage, selectedResumeMode]);

  const hasCustomSectionsSelected = selectedCustomSections.length > 0;
  const canOpenResume =
    selectedResumeMode === "complete" ||
    (selectedResumeMode === "custom" && hasCustomSectionsSelected);
        ...
        {isResumeCardOpen ? (
          <div id="resume-options-panel" className="resume-options-card">
            <div className="resume-options-group">
              <p className="resume-options-label">
                {footer.resumeLanguagesLabel}
              </p>
                ...
                {resumeLanguages.map((language) => (
                  <button
                    ...
                    className={`resume-chip ${
                      selectedResumeLanguage === language.code
                        ? "is-active"
                        : ""
                    }`}
            ...
            <div className="resume-options-group">
              ...
              <div className="resume-mode-grid">
                {Object.entries(footer.resumeModes).map(([modeKey, mode]) => (
                  ...
                  >
                    <span
                      className="resume-mode-selected-icon"
                      aria-hidden="true"
                    >
                      ✓
                    </span>
            ...
            {selectedResumeMode === "custom" ? (
              <div className="resume-options-group resume-custom-builder">
                <p className="resume-custom-fixed">
                  {footer.customSectionsHint}
                </p>

                <p className="resume-options-label">
                  {footer.customSectionsLabel}
                </p>
                <div className="resume-custom-checklist">
                  {Object.entries(footer.customSections).map(
                    ([sectionKey, section]) => {
                      const isSelected =
                        selectedCustomSections.includes(sectionKey);
                      ...
                      return (
                        <label key={sectionKey} className="resume-check-row">
                          <input
                            ...
                            checked={isSelected}
                            onChange={() =>
                              handleToggleCustomSection(sectionKey)
                            }
                          />
                ...
                <div className="resume-custom-summary">
                  <p className="resume-options-label">
                    {footer.customSectionsOptionsLabel}
                  </p>
                  ...
                  {visibleCustomOptions.length ? (
                    ...
                    <p className="resume-custom-empty">
                      {footer.customSectionsOptionsEmpty}
                    </p>
                  )}
                </div>
              </div>
